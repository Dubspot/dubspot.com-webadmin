...
<h1>All Course_instances</h1>

<p>{{ link_to_route('course_instances.create', 'Add new course_instance') }}</p>

@if ($course_instances->count())
	<table class="table table-striped table-bordered">
		...
		<tbody>
			...
			<tr>
				...
				<td>{{ link_to_route('course_instances.show', 'Show', array($course_instance->id), array('class' => 'btn btn-info')) }}</td>
	...
	{{ $course_instances->links() }}

@else
	There are no course_instances
@endif
