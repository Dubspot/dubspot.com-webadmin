...
<h1>All Authorize_responses</h1>

<p>{{ link_to_route('authorize_responses.create', 'Add new authorize_response') }}</p>

@if ($authorize_responses->count())
	<table class="table table-striped table-bordered">
		...
		<tbody>
			@foreach ($authorize_responses as $authorize_response)
				<tr>
          ...
          <td>{{ link_to_route('authorize_responses.show', 'Show', array($authorize_response->id), array('class' => 'btn btn-info')) }}</td>
	...
	{{ $authorize_responses->links() }}

@else
	There are no authorize_responses
@endif
