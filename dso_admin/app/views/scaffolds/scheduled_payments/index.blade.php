...
<h1>All Scheduled_payments</h1>

<p>{{ link_to_route('scheduled_payments.create', 'Add new scheduled_payment') }}</p>

@if ($scheduled_payments->count())
	<table class="table table-striped table-bordered">
		...
		<tbody>
			...
			<tr>
				...
				<td>{{ link_to_route('scheduled_payments.show', 'Show', array($scheduled_payment->id), array('class' => 'btn btn-info')) }}</td>
	...
	{{ $scheduled_payments->links() }}

@else
	There are no scheduled_payments
@endif
